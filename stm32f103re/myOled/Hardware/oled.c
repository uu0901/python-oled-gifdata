...
//支持逐行式，适配u8g2取模顺序，但是oled_data里的取模数据仍是列行式，也就是开启这个宏，你只能使用gif显示的功能..懒得做适配了，打算把gif数组数据做列行式扫描而不是逐行式，因此这个宏是做功能测试
//#define OLED_ROW_MODE

#ifdef OLED_ROW_MODE
uint8_t OLED_DisplayBuf[64][128/8]; // 逐行式
#else
uint8_t OLED_DisplayBuf[8][128]; // 页式（原版）
#endif
//64*128像素
//列行式
//y轴每8位一字节，分8页，因此64/8=8 x轴128列

//逐行式
//x轴每8位一字节，因此128/8=16 y轴64行
//uint8_t OLED_DisplayBuf[64][16];
/*********************全局变量*/
...
void oledUpdate(void)
{
	...
	for (uint8_t page = 0; page < 8; page++)
	{
		...
		if (OLED_DisplayBuf[page*8 + bit][x / 8] & (0x80 >> (x % 8)))
		{
			byte |= (1 << bit);
		}
	}
...
void OLED_UpdateArea(int16_t x, int16_t y, uint8_t w, uint8_t h)
{
	...
	for (uint8_t page = pageStart; page <= pageEnd; page++)
	{
		for (uint8_t i = x; i < x + w; i++)
		{
			...
			for (uint8_t bit = 0; bit < 8; bit++)
			{
				uint8_t yy = page * 8 + bit;
				if (yy >= y && yy < y + h)
				{
					if (OLED_DisplayBuf[yy][i / 8] & (0x80 >> (i % 8)))
					{
						byte |= (1 << bit);
					}
				}
		...
		OLED_WriteData(pageBuf, w);
	}
#else
	for (uint8_t page = y / 8; page < (y + h + 7) / 8; page++)
	{
		OLED_SetCursor(page, x);
		OLED_WriteData(&OLED_DisplayBuf[page][x], w);
	}
#endif
	// int16_t j;
	// int16_t Page, Page1;
...
void OLED_ClearArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			...
			{
				#ifdef OLED_ROW_MODE
				OLED_DisplayBuf[j][i / 8] &= ~(0x80 >> (i % 8));
				#else
...
void oledReverse(void)
{
    ...
    {
        for (uint8_t bx = 0; bx < 128 / 8; bx++)
        {
            OLED_DisplayBuf[y][bx] ^= 0xFF;
        }
    }
#else
    // 页式：8 页 × 128 列
...
void oledReverseArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
    ...
    for (j = Y; j < Y + Height; j++)
    {
        for (i = X; i < X + Width; i++)
        {
            ...
            {
#ifdef OLED_ROW_MODE
                OLED_DisplayBuf[j][i / 8] ^= 0x80 >> (i % 8); // 逐行式
#else
                OLED_DisplayBuf[j / 8][i] ^= 0x01 << (j % 8); // 页式（原始正确写法）
#endif
            }
        }
    }
...
//		for (i = 0; i < Width; i++)
//		{
//			if (X + i >= 0 && X + i <= 127 && Y + j >= 0 && Y + j <= 63)
//			{
//				#ifdef OLED_ROW_MODE
//				// 逐行式：每字节 8 水平像素
//				if (Image[j * Width / 8 + i / 8] & (0x80 >> (i % 8)))
//				{
//					OLED_DisplayBuf[Y + j][(X + i) / 8] |= 0x80 >> ((X + i) % 8);
//				}
//				else
//				{
//					OLED_DisplayBuf[Y + j][(X + i) / 8] &= ~(0x80 >> ((X + i) % 8));
...
#ifdef OLED_ROW_MODE
    // 逐行模式：每字节 8 水平像素，LSB 左起
    uint8_t bytes_per_row = width / 8;
    for (uint8_t row = 0; row < height; row++)
    {
        for (uint8_t byte_idx = 0; byte_idx < bytes_per_row; byte_idx++)
        {
            uint8_t data = image[row * bytes_per_row + byte_idx];

            for (uint8_t bit = 0; bit < 8; bit++)
            {
                uint8_t pixel = (data >> bit) & 0x01; // LSB → MSB
                uint8_t col = x + byte_idx * 8 + bit;
                uint8_t line = y + row;

                if (pixel)
                    OLED_DisplayBuf[line][col / 8] |=  (0x80 >> (col % 8));
                else
                    OLED_DisplayBuf[line][col / 8] &= ~(0x80 >> (col % 8));
            }
        }
...
#else
    // 页模式：每字节 8 垂直像素，bit0 顶部
    uint8_t pages = (height + 7) / 8;
    for (uint8_t page = 0; page < pages; page++)
    {
        for (uint8_t col = 0; col < width; col++)
        {
            uint8_t data = image[page * width + col];
            for (uint8_t bit = 0; bit < 8; bit++)
            {
                uint8_t pixel = (data >> bit) & 0x01; // bit0 顶部
                uint8_t yy = y + page * 8 + bit;
                if (yy >= y + height) break; // 防止超出范围

                if (pixel)
                    OLED_DisplayBuf[yy / 8][x + col] |=  (1 << (yy % 8));
                else
                    OLED_DisplayBuf[yy / 8][x + col] &= ~(1 << (yy % 8));
            }
        }
...
#ifdef OLED_ROW_MODE
                // 逐行模式：LSB 左起
                uint8_t bit = (Image[j * Width / 8 + i / 8] & (0x01 << (i % 8))) ? 1 : 0;
                if (Invert) bit = !bit;
...
#else
                // 页模式：bit0 顶部
                uint8_t bit = (Image[(j / 8) * Width + i] & (0x01 << (j % 8))) ? 1 : 0;
                if (Invert) bit = !bit;

                if (bit)
                    OLED_DisplayBuf[(Y + j) / 8][X + i] |=  (1 << ((Y + j) % 8));
                else
                    OLED_DisplayBuf[(Y + j) / 8][X + i] &= ~(1 << ((Y + j) % 8));
#endif
            }
...
//		OLED_DisplayBuf[Y / 8][X] |= 0x01 << (Y % 8);
		//OLED_DisplayBuf[Y][X / 8] |= 0x80 >> (X % 8);
...
//		}
#ifdef OLED_ROW_MODE
		return (OLED_DisplayBuf[Y][X / 8] & (0x80 >> (X % 8))) ? 1 : 0;
#else
